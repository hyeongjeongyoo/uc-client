...
interface NiceAuthCallbackPostMessage {
  ...
  userData?: NiceRegisteredUserDataDto; // Though Find ID doesn't use userData, keep for type consistency
  error?: string | null;
  errorCode?: string | null;
  status?: string | null; // Backend's actual status: e.g. "ID_SENT", "ACCOUNT_NOT_FOUND"
  email?: string | null; // Masked email if ID_SENT
  message?: string | null; // Message from backend
}

type PageStatus =
  | "idle"
  | "auth_popup_initiated" // Waiting for user to complete NICE in popup
  | "processing_auth_result" // NICE auth done, processing with backend (not really applicable here as backend does it)
  | "id_sent" // ID successfully found and info sent (e.g. via email)
  | "account_not_found"
  | "auth_fail_from_popup" // Generic NICE failure from popup
  | "error_from_initiation" // Error opening popup or initiating NICE
  | "error_from_callback"; // Error message explicitly from callback parameters
...
const NICE_POPUP_NAME_FIND_ID = "niceAuthPopup_FindId";

// No mockFindIdApi needed as backend handles ID finding post-NICE and redirects to callback page
// The callback page then postMessages the final status like "ID_SENT" or "ACCOUNT_NOT_FOUND".
...
function FindIdContent() {
  ...
  > = {
    onSuccess: (data: NiceInitiateResponse) => {
      ...
      // reqSeq can be stored if needed for validation, but not critical for this flow
      ...
      if (!window.name) window.name = "Parent_Window_Arpina_FindId";
      const nicePopup = window.open(
        "about:blank",
        NICE_POPUP_NAME_FIND_ID,
      ...
      form.setAttribute("target", NICE_POPUP_NAME_FIND_ID);
    ...
    onError: (error: Error) => {
      console.error("NICE Auth Initiation Error (Find ID):", error);
      toaster.create({
        title: "본인인증 실행 오류",
  ...
  const initiateNiceMutation = useMutation<
    ...
    unknown
  >({
    mutationFn: () => niceApi.initiateVerification({ serviceType: "FIND_ID" }),
    ...mutationOptions,
  });
  ...
  useEffect(() => {
    const handleNiceCallbackMessage = (event: MessageEvent) => {
      if (event.origin !== window.location.origin) {
        console.warn(
          "Find ID: Message from unexpected origin ignored:",
          event.origin
        );
        return;
      }
      ...
      if (
        ...
        messageData.niceServiceType === "FIND_ID"
      ) {
        const nicePopup = window.open("", NICE_POPUP_NAME_FIND_ID);
        if (nicePopup && !nicePopup.closed) nicePopup.close();

        if (
          messageData.type === "NICE_AUTH_OTHER" &&
          messageData.status === "ID_SENT"
        ) {
          toaster.create({
            title: "아이디 찾기 완료",
            description:
              messageData.message || "아이디 정보가 이메일로 발송되었습니다.",
            type: "success",
          });
          setPageStatus("id_sent");
          setStatusDisplay({
            ...
            message: (
              <VStack align="stretch" gap={1}>
                <Text>
                  회원가입시 입력하신 이메일 주소로 아이디를 전송했습니다.
                </Text>
          ...
          // Other NICE_AUTH_OTHER statuses or unexpected types for FIND_ID
          toaster.create({
            title: "처리 오류",
            description:
              messageData.message ||
              "아이디 찾기 처리 중 알 수 없는 상태값 또는 오류가 발생했습니다.",
            type: "error",
          });
  ...
  }, []); // No dependencies that would require re-running this for find-id
      ...
      case "error_from_callback":
        if (!statusDisplay?.message) {
          setStatusDisplay({
            image: "/images/find-credentials/mail_fail.png",
            title: "오류 발생",
            message: "아이디 찾기 처리 중 알 수 없는 오류가 발생했습니다.",
            showHomeButton: true,
            showRetryButton: true,
          });
        }
  ...
  const handleInitiateNiceAuth = () => {
    // Reset status before initiating
    setPageStatus("idle");
    setStatusDisplay(null);
