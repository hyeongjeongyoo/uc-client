...
uniform int uGradientAxis; // 0:x, 1:y
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (uUseGradient == 1) {
      float t = (uGradientAxis == 0) ? uv.x : uv.y;
      vec3 base = mix(uColorStart, uColorEnd, clamp(t, 0.0, 1.0));
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    const [cr, cg, cb] = normalizeColor(color);
    const useGrad = Array.isArray(gradient) && gradient.length === 2;
    const [gr1, gg1, gb1] = useGrad
      ? normalizeColor(gradient![0])
      : [cr, cg, cb];
    const [gr2, gg2, gb2] = useGrad
      ? normalizeColor(gradient![1])
      : [cr, cg, cb];
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorStart: { value: new Color(gr1, gg1, gb1) },
        uColorEnd: { value: new Color(gr2, gg2, gb2) },
        uUseGradient: { value: useGrad ? 1 : 0 },
        uGradientAxis: { value: gradientAxis === "y" ? 1 : 0 },
